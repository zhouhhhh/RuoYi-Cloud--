...
里程碑 03 已完成客户表结构、代码生成、人工修正、菜单与字典、列表、
分页、筛选、新增、修改、详情、单条和批量逻辑删除，详细证据见
`docs/progress/milestone-03.md`。

当前只推进手机号后端校验这一小闭环：

1. 使用非法格式和重复手机号建立当前失败行为基线，记录响应和数据库状态。
2. 参考现有 RuoYi 参数校验写法，明确前端校验、Bean Validation 和数据库
   唯一索引各自负责的边界。
3. 为新增和修改接口增加手机号必填、格式校验，并验证明确的错误响应。
4. 保留 `uk_customer_phone` 作为并发条件下的最终唯一约束。

本轮暂不实现客户状态流转规则，不同时展开里程碑 04 的全部内容。
