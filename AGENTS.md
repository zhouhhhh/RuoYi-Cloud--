...
本项目已配置 IntelliJ IDEA MCP Server，用于在支持且已加载该工具的
Codex 对话中辅助排错。

- 可用于读取运行配置、构建结果、启动输出、文件诊断和调用链。
- 启动或停止服务、执行 SQL、修改代码等有状态操作，仍须先获得学习者明确授权。
- MCP 服务不可用或当前对话未加载其工具时，退回到学习者提供的 IDEA 控制台、
  Network、Nacos、Redis、MySQL 与 Docker 证据。
- 不将 MCP 服务版本或本机监听端口作为固定项目事实；IDEA 升级后这些信息可能变化。
...
当前阶段：`04 业务校验` 已于 2026-09-02 按修订后的范围验收完成并归档。

已完成手机号必填、格式、普通重复预查、数据库唯一约束兜底及唯一冲突
异常转换，以及客户状态非空和合法值校验。运行证据、构建结果、学习结论
和验证边界见 `docs/progress/milestone-04.md`。

客户状态规则以学习者于 2026-09-02 确认的业务需求为准：

- `status` 必填，只允许 `0 潜在`、`1 跟进中`、`2 已成交`、`3 已流失`。
- 新增可以使用任意合法状态，默认 `0` 不等于强制只能为 `0`。
- 合法状态之间允许自由修改和纠正，不实施单向流转、终态锁定或管理员审批。
- 继续保留现有新增、修改权限，不因允许状态纠正而放开权限。

原计划中的严格状态流转限制已取消，不再作为里程碑 04 的待办或验收缺项。
手机号唯一索引 `uk_customer_phone` 继续保留；手工绕过预查的兜底验证不等于
已经完成真实并发测试，`-DskipTests` 打包也不代表自动化测试通过。

下一里程碑为 `05 联系人管理`，目前待开始。本次只完成归档，不展开其实现；
学习者下次要求继续时，再按 AI Tutor Mode 选择一个 30 至 90 分钟的小闭环。
...
- **状态：** [x]（2026-09-02，按修订后的业务范围验收）
- **开发内容：** 手机号唯一、必填和格式校验、客户状态合法值校验，以及客户唯一冲突的业务异常转换。
- **必须理解：** 前后端校验边界、Bean Validation、数据库唯一索引、竞态条件、异常转换，以及状态取值约束与流转规则的区别。
- **完成标准：** 手机号为空、格式错误、重复手机号及空或非法状态被后端拒绝并返回明确错误；新增允许任意合法状态，合法状态可自由修改和纠正。
- **验证证据：** 正常与失败请求、唯一索引、异常日志、数据库前后对照及构建结果，详见 `docs/progress/milestone-04.md`；本次为手工功能验收，不宣称自动化或真实并发测试通过。
